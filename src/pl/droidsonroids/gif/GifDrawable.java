...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
    ...
    public void draw(Canvas canvas) {
        ...
        if (mPaint.getShader() == null) {
            ...
            if (colors != null){
                if(mIsCircle && mMetaData[0] != 0 && mMetaData[1] != 0){
                    drawRoundBitmap(canvas, Bitmap.createBitmap(colors, mMetaData[0], mMetaData[1], Bitmap.Config.ARGB_8888));
                }else{
    ...
    private int mBorderThickness;   //外环宽度
    private void drawRoundBitmap(Canvas canvas, Bitmap bitmap) {
        try {
            float radius = (float)Math.min(mMetaData[0], mMetaData[1]) / 2 - mBorderThickness;
            int width = bitmap.getWidth();
            int height = bitmap.getHeight();
            float left = 0;
            float top = 0;
            if(Math.max(width, height) == width){
                left = (float)(width - height)/2;
            }else{
                top = (float)(height - width)/2;
            }
            ...
            if(mBorderThickness != 0){
                drawCircleBorder(canvas, radius, mBorderInsideColor);   //圆环画外面
            }
        } catch (Exception e) {
        }
    }
    ...
    private void drawCircleBorder(Canvas canvas, float radius, int color) {
        ...
        canvas.drawCircle((float)(mDstRect.left + mDstRect.right) / 2, (float)(mDstRect.top + mDstRect.bottom) / 2, radius, paint);
    }
    //TODO:
